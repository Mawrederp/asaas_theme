	...
	make_standard_filters: function make_standard_filters() {
		...
		if (this.meta) {
			...
			if (this.is_list_view) {
				$('<span class="octicon octicon-search text-muted small"></span>').prependTo(this.page.page_form);
			}
			this.page.add_field({
				fieldtype: 'Data',
				label: 'ID',
...
frappe.ui.FilterList = Class.extend({
	...
	get_formatted_value: function get_formatted_value(field, val) {
		var value = val;
...
});frappe.templates['edit_filter'] = '<div class="filter-box">  <div class="list_filter row">   <div class="fieldname_select_area col-sm-4 col-sm-4 form-group ui-front"></div>   <div class="col-sm-2 form-group">    <select class="condition form-control">     <option value="=">{%= __("Equals") %}</option>     <option value="like">{%= __("Like") %}</option>     <option value="in">{%= __("In") %}</option>     <option value="not in">{%= __("Not In") %}</option>     <option value="!=">{%= __("Not Equals") %}</option>     <option value="not like">{%= __("Not Like") %}</option>     <option value=">">{%= __(">") %}</option>     <option value="<">{%= __("<") %}</option>     <option value=">=">{%= __(">=") %}</option>     <option value="<=">{%= __("<=") %}</option>     <option value="Between">{%= __("Between") %}</option>    </select>   </div>   <div class="col-sm-3 col-sm-4  col-xs-12">    <div class="filter_field pull-left"></div>        <div class="clearfix"></div>   </div>   <div class="filter-actions pull-left col-sm-1">    <a class="set-filter-and-run btn btn-sm btn-primary pull-left">     <i class=" fa fa-check visible-xs"></i>     <span class="hidden-xs">{%= __("Apply") %}</span></a>   </div>   <div class="filter-actions pull-left col-sm-1">    <a class="small grey remove-filter pull-left">     <i class="octicon octicon-trashcan visible-xs"></i>     <span class="hidden-xs">{%= __("Remove") %}</span></a>   </div>  </div> </div> ';
...
frappe.ui.Tags = function () {
	function _class(_ref) {
		var parent = _ref.parent,
		    placeholder = _ref.placeholder,
		    tagsList = _ref.tagsList,
		    onTagAdd = _ref.onTagAdd,
	...
	_createClass(_class, [{
		key: 'setup',
		value: function setup(parent, placeholder) {
		...
		value: function removeTag(label) {
			if (this.tagsList.includes(label)) {
				var $tag = this.$ul.find('.frappe-tag[data-tag-label="' + label + '"]');
				$tag.remove();
				this.tagsList = this.tagsList.filter(function (d) {
					return d !== label;
				});
		...
		key: 'getListElement',
		value: function getListElement($element, className) {
		...
		value: function getTag(label) {
			...
			$removeTag.on("click", function () {
				_this2.removeTag($removeTag.attr('data-tag-label'));
			});

			if (this.onTagClick) {
				var $toggle_tag = $tag.find(".toggle-tag");
				$toggle_tag.on("click", function () {
					_this2.onTagClick($toggle_tag.attr('data-tag-label'));
				});
			}
...
frappe.ui.TagEditor = Class.extend({
	...
	setup_tags: function setup_tags() {
		var me = this;
		...
		this.tags = new frappe.ui.Tags({
			...
			onTagAdd: function onTagAdd(tag) {
			...
			onTagRemove: function onTagRemove(tag) {
				if (!me.refreshing) {
					return frappe.call({
		...
		$input.on("input", function (e) {
			...
			frappe.call({
				...
				args: {
					...
					cat_tags: me.list_sidebar ? JSON.stringify(me.list_sidebar.get_cat_tags()) : '[]'
				},
				callback: function callback(r) {
...
frappe.views.ListSidebar = Class.extend({
	...
	make_kanban_board: function make_kanban_board(board_name, field_name, project) {
		var me = this;
		return frappe.call({
	...
	setup_calendar_view: function setup_calendar_view() {
		var _this = this;
		...
		frappe.db.get_list('Calendar View', {
			...
			var $link_calendar = _this.sidebar.find('.list-link[data-view="Calendar"]');

			var default_link = '';
			if (frappe.views.calendar[_this.doctype]) {
				default_link = '<li><a href="#List/' + doctype + '/Calendar/Default">\n\t\t\t\t\t' + __("Default") + '</a></li>';
			}
			var other_links = calendar_views.map(function (calendar_view) {
				return '<li><a href="#List/' + doctype + '/Calendar/' + calendar_view.name + '">\n\t\t\t\t\t' + __(calendar_view.name) + '</a>\n\t\t\t\t</li>';
			}).join('');
	...
	setup_email_inbox: function setup_email_inbox() {
		...
		accounts.forEach(function (account) {
			...
			if (!divider) {
				_this2.get_divider().appendTo($dropdown);
				divider = true;
			}
	...
	get_divider: function get_divider() {
		return $('<li role="separator" class="divider"></li>');
	}
});frappe.templates['list_sidebar'] = '<ul class="list-unstyled sidebar-menu user-actions hide">  <li class="divider"></li> </ul> <ul class="list-unstyled sidebar-menu standard-actions">   {% if frappe.model.can_get_report(doctype) %}   <li class="divider"></li>   <li>    <div class="btn-group">     <a class="dropdown-toggle green" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">        {{ __("Reports") }} <span class="caret"></span>      </a>     <ul class="dropdown-menu reports-dropdown" style="max-height: 300px; overflow-y: auto;">      <li><a href="#Report/{{ doctype }}">{{ __("Report Builder") }}</a></li>     </ul>    </div>   </li>  {% endif %}  <li class="divider"></li>  <li class="list-link" data-view="List">   <a class="orange" href="#List/{%= doctype %}/List">{%= __("List") %}</a></li>  <li class="hide list-link" data-view="Image">   <a class="grey" href="#List/{%= doctype %}/Image">{%= __("Images") %}</a></li>  <li class="hide list-link" data-view="Gantt">   <a href="#List/{%= doctype %}/Gantt">{%= __("Gantt") %}</a></li>  <li class="hide tree-link">   <a class="yellow" href="#Tree/{%= doctype %}">{%= __("Tree") %}</a></li>  <li class="hide list-link" data-view="Calendar">   <a href="#List/{%= doctype %}/Calendar">{%= __("Calendar") %}</a></li>  <li class="hide list-link" data-view="Kanban">   <div class="btn-group">    <a class="dropdown-toggle orange" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">     {{ __("Kanban") }} <span class="caret"></span>    </a>    <ul class="dropdown-menu kanban-dropdown" style="max-height: 300px; overflow-y: auto;">     <li class="new-kanban-board"><a>{{ __("New Kanban Board") }}</a></li>    </ul>   </div>  </li>  <li class="hide list-link" data-view="Inbox">   <div class="btn-group">    <a class="dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">     {{ __("Email Inbox") }} <span class="caret"></span>    </a>    <ul class="dropdown-menu email-account-dropdown" style="max-height: 300px; overflow-y: auto;">    </ul>   </div>  </li>  <li class="assigned-to-me">   <a class="yellow">{%= __("Assigned To Me") %}</a>  </li>  {% if(frappe.help.has_help(doctype)) { %}  <li><a class="help-link" data-doctype="{{ doctype }}">{{ __("Help") }}</a></li>  {% } %} </ul> <ul class="list-unstyled close-sidebar-button visible-xs visible-sm">  <li class="divider"></li>  <li class="close-sidebar">Close</li> </ul> ';
frappe.templates['list_sidebar_stat'] = '<ul class="list-unstyled sidebar-menu sidebar-stat">  <li class="divider"></li>  <li class="h6 stat-label">{{ label }}</li>  {% if(!stat.length) { %}  <li class="stat-no-records text-muted">{{ __("No records tagged.") }}</li>  {% } else {   for (var i=0, l=stat.length; i < l; i++) {    var stat_label = stat[i][0];    var stat_count = stat[i][1];  %}    <li>     <a class="stat-link badge-hover" data-label="{{ stat_label %}" data-field="{{ field %}">      <span class="badge">{{ stat_count }}</span>      <span>{{ __(stat_label) }}</span>     </a>    </li>   {% }  } %} </ul> <div class="show-tags-btn">  <a class="list-tag-preview hidden-xs text-muted">{{ __("Show tags") }}</a> </div> ';
...
frappe.views.ListRenderer = Class.extend({
	...
	render_count: function render_count() {
		var $header_right = this.list_view.list_header.find('.list-item__content--activity');
		var current_count = this.list_view.data.length;
		...
		frappe.call({
			...
			var count = r.message.values[0][0] || current_count;
			var str = __('{0} of {1}', [current_count, count]);
	...
	set_title_with_row_number: function set_title_with_row_number(data, id) {
...
frappe.views.GanttView = frappe.views.ListRenderer.extend({
	...
	set_colors: function set_colors() {
		var classes = this.tasks.map(function (t) {
			return t.custom_class;
		}).filter(function (c) {
			return c && c.startsWith('color-');
		});
	...
	set_columns: function set_columns() {},
	required_libs: ["assets/frappe/js/lib/snap.svg-min.js", "assets/frappe/js/lib/frappe-gantt/frappe-gantt.css", "assets/frappe/js/lib/frappe-gantt/frappe-gantt.min.js"]
});
...
frappe.views.CalendarView = frappe.views.ListRenderer.extend({
	...
	render_view: function render_view() {
		...
		var me = this;
		this.get_calendar_options().then(function (options) {
			_this.calendar = new frappe.views.Calendar(options);
		});
	},
	load_last_view: function load_last_view() {
		var route = frappe.get_route();
		...
		if (!route[3]) {
			var calendar_view = this.user_settings.last_calendar_view;
	...
	get_calendar_view: function get_calendar_view() {
		return frappe.get_route()[3];
	},
	get_calendar_options: function get_calendar_options() {
		var _this2 = this;

		var calendar_view = frappe.get_route()[3] || 'Default';
		...
		return new Promise(function (resolve) {
			if (calendar_view === 'Default') {
				Object.assign(options, frappe.views.calendar[_this2.doctype]);
				resolve(options);
			} else {

				frappe.model.with_doc('Calendar View', calendar_view, function () {
					var doc = frappe.get_doc('Calendar View', calendar_view);
...
frappe.views.Calendar = Class.extend({
	...
	prepare_colors: function prepare_colors(d) {
		var color = void 0,
		    color_name = void 0;
...
frappe.views.ImageView = frappe.views.ListRenderer.extend({
	...
	render_view: function render_view(values) {
		...
		this.get_attached_images().then(function () {
			_this.render_image_view();

			if (!_this.gallery) {
				_this.setup_gallery();
			} else {
				_this.gallery.prepare_pswp_items(_this.items, _this.images_map);
			}
		});
	},
	set_defaults: function set_defaults() {
	...
	get_attached_images: function get_attached_images() {
		var _this2 = this;

		return frappe.call({
			method: 'frappe.core.doctype.file.file.get_attached_images',
			args: { doctype: this.doctype, names: this.items.map(function (i) {
					return i.name;
				}) }
		}).then(function (r) {
			_this2.images_map = Object.assign(_this2.images_map || {}, r.message);
		});
	},
	get_header_html: function get_header_html() {
...
frappe.views.GalleryView = Class.extend({
	...
	prepare_pswp_items: function prepare_pswp_items(_items, _images_map) {
		var _this3 = this;
		...
		return new Promise(function (resolve) {
			var items = _this3.items.map(function (i) {
	...
	show: function show(docname) {
		...
		this.lib_ready.then(function () {
			return _this4.prepare_pswp_items();
		}).then(function () {
			return _this4._show(docname);
		});
	},
	_show: function _show(docname) {
		var me = this;
		var items = this.pswp_items;
		var item_index = items.findIndex(function (item) {
		...
		var options = {
			...
			getThumbBoundsFn: function getThumbBoundsFn(index) {
				var query = 'img[data-name="' + items[index].name + '"]';
	...
	browse_images: function browse_images() {
		...
		$more_items.on('click', '.pswp__more-item', function (e) {
			var img_el = e.target;
			var index = _this5.pswp.items.findIndex(function (i) {
				return i.name === _this5.pswp.currItem.name;
			});

			_this5.pswp.goTo(index);
			_this5.pswp.items.splice(index, 1, {
				src: img_el.src,
				w: img_el.naturalWidth,
				h: img_el.naturalHeight,
				name: _this5.pswp.currItem.name
			});
			_this5.pswp.invalidateCurrItems();
			_this5.pswp.updateSize(true);
		});
	...
	load_lib: function load_lib() {
		return new Promise(function (resolve) {
			...
			frappe.require([asset_dir + 'photoswipe.css', asset_dir + 'default-skin.css', asset_dir + 'photoswipe.js', asset_dir + 'photoswipe-ui-default.js'], resolve);
		});
	}
});
...
frappe.views.TreeView = Class.extend({
	...
	print_tree: function print_tree() {
		if (!frappe.model.can_print(this.doctype)) {
	...
	set_menu_item: function set_menu_item() {
		...
		me.page.add_menu_item(__('Add to Desktop'), function () {
			var label = me.doctype === 'Account' ? __('Chart of Accounts') : __(me.doctype);
